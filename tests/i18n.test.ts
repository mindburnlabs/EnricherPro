...
describe('i18n Implementation', () => {
  ...
  it('should handle Russian plurals correctly (research.conflict)', () => {
    // one: 1, 21, 31...
    expect(i18next.t('research:conflict.detected_plural', { count: 1 })).toBe(
      'Обнаружена 1 аномалия',
    );

    // few: 2, 3, 4...
    expect(i18next.t('research:conflict.detected_plural', { count: 2 })).toBe(
      'Обнаружено 2 аномалии',
    );

    // many: 5, 0, 11...
    expect(i18next.t('research:conflict.detected_plural', { count: 5 })).toBe(
    ...
    files.forEach((file) => {
      ...
      while ((match = tRegex.exec(content)) !== null) {
        ...
        if (!key.includes(':')) {
          ...
          if (!allKeys.has(`common:${key}`) && !allKeys.has(`:${key}`)) {
            // :key is invalid but just in case
