...
export async function withRetry<T>(
    ...
    while (true) {
        try {
            ...
            if (status === 429) {
                ...
                if (resetMatch && resetMatch[1]) {
                    ...
                    if (!isNaN(resetDate.getTime())) {
                        ...
                        if (timeToWait > 0) {
                            ...
                            console.warn(`[Smart Rate Limit] Hit 429. Pause until ${resetDate.toISOString()} (${Math.round(finalDelay / 1000)}s)...`);
                        }
                    }
                }
                ...
                if (finalDelay === 0) {
                    ...
                    if (retryAfterMatch && retryAfterMatch[1]) {
                        ...
                        console.warn(`[Smart Rate Limit] Hit 429. Retry after ${retryAfterMatch[1]}s detected.`);
                    }
                }
                ...
                if (finalDelay === 0 && error.response?.headers) {
                    ...
                    if (retryAfter) {
                        ...
                        if (finalDelay > 0) console.warn(`[Smart Rate Limit] Hit 429. Retry-After header: ${Math.round(finalDelay / 1000)}s`);
                    }
                }
            }
            ...
            if (status !== 429) {
                console.warn(`[Retry] Attempt ${attempt}/${maxRetries} failed. Retrying in ${Math.round(finalDelay)}ms...`, error.message);
            }
