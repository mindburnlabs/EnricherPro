                ...
                <div className="p-6">
                    {step === 'checklist' ? (
                        <div className="space-y-6">
                            {/* Channel Selector */}
                            <div>
                                <label className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 block">Target Marketplace</label>
                            ...
                            <div className="space-y-3">
                                {validations.map((v, i) => (
                                    <div key={i} className="flex items-center justify-between text-sm p-3 rounded-lg border border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/30">
                                        <span className="font-medium text-gray-700 dark:text-gray-300">{v.label}</span>
                                        {v.status === 'pass' && <span className="text-emerald-500 flex items-center gap-1 font-bold text-xs"><CheckCircle size={14}/> PASS</span>}
                                        {v.status === 'warn' && <span className="text-amber-500 flex items-center gap-1 font-bold text-xs"><AlertTriangle size={14}/> WARNING</span>}
                                        {v.status === 'fail' && <span className="text-red-500 flex items-center gap-1 font-bold text-xs"><XCircle size={14}/> BLOCKER</span>}
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="pt-4 flex gap-3">
                                <button onClick={onClose} className="flex-1 py-3 text-sm font-bold text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl transition-colors">
                                    Cancel
                                </button>
                                <button 
                                ...
                                >
                                    {isPublishing ? "Publishing..." : hasErrors ? "Fix Blockers First" : hasWarnings ? "Publish with Warnings" : "Publish Now"}
                                </button>
                            </div>
                        </div>
                    ) : (
                        <div className="text-center py-8 space-y-6 animate-in fade-in slide-in-from-bottom-4">
                            ...
                            <div>
                                <h3 className="text-2xl font-bold text-gray-900 dark:text-white">Successfully Published!</h3>
                            ...
                            <div className="flex gap-3 justify-center">
                                ...
                                <button onClick={handleDownload} className="px-4 py-2 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-lg text-sm font-medium flex items-center gap-2 hover:bg-gray-200 transition">
