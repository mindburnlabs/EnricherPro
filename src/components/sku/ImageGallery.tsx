...
export const ImageGallery: React.FC<ImageGalleryProps> = ({ images = [] }) => {
  ...
  const displayImages =
    images.length > 0
      ...
      : ([
          {
            url: 'https://placehold.co/600x400/png?text=Product+Image',
            alt: 'Placeholder 1',
            status: 'ok',
          },
          {
            url: 'https://placehold.co/600x400/png?text=Side+View',
            alt: 'Placeholder 2',
            status: 'pending',
          },
          {
            url: 'https://placehold.co/600x400/png?text=Datasheet+Snippet',
            alt: 'Datasheet',
            status: 'flagged',
          },
        ] as any[]);
        ...
        {displayImages.map((img, idx) => (
          ...
          >
            ...
            <div className='absolute top-1 right-1'>
              {img.status === 'ok' && (
                <CheckCircle size={14} className='text-emerald-500 fill-white dark:fill-gray-900' />
              )}
              {img.status === 'flagged' && (
                <AlertOctagon size={14} className='text-red-500 fill-white dark:fill-gray-900' />
              )}
            </div>
          </div>
        ))}
