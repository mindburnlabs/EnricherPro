...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ initialJobId, onJobCreated }) => {
  ...
  const handleSearch = async (
    ...
    if (!config.apiKeys.firecrawl || !config.apiKeys.openRouter) {
      alert(
        t(
          'errors.missing_keys',
          'Please configure Firecrawl and OpenRouter API keys in Settings to proceed.',
        ),
      );
      return;
    }
    ...
    try {
      ...
      if (res.success && res.jobId) {
        setMessages((prev) =>
          prev.map((m) => (m.id === placeholderId ? { ...m, id: res.jobId, jobId: res.jobId } : m)),
        );
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m.id === placeholderId ? { ...m, status: 'failed', error: 'Failed to start job' } : m,
          ),
        );
      }
    } catch (e) {
  ...
  const handleMerge = (item: EnrichedItem) => {
    ...
    const mockCurrent = {
      ...item,
      id: 'existing-db-item',
      data: { ...item.data, mpn_identity: { ...item.data.mpn_identity, mpn: 'OLD-MPN' } },
    };
  ...
  return (
    <div className='flex h-full w-full relative overflow-hidden' ref={splitContainerRef}>
      ...
      >
          ...
          <div className='pt-8 space-y-8 px-4'>
            {messages.map((msg, idx) => (
              ...
              >
                ...
                <div
                  className={`flex-1 max-w-3xl space-y-3 ${msg.role === 'user' ? 'text-right' : ''}`}
                >
        ...
        {/* Input Area */}
        <div
          className={`transition-all duration-500 ease-in-out z-40 ${isEmpty ? 'flex-1 flex flex-col justify-center mb-[20vh]' : 'sticky bottom-0 pb-6 pt-4 bg-gradient-to-t from-white via-white to-transparent dark:from-gray-900 dark:via-gray-900'}`}
        >
          {isEmpty && (
