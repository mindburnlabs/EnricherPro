...
export function ObservabilityDashboard({
  ...
  const COST_THRESHOLDS = {
    warning: 5.0, // $5.00 - yellow warning
    critical: 20.0, // $20.00 - red critical
    daily: 2.0, // $2.00/day average
  };
      ...
      <div className='flex items-center justify-between'>
        <div className='flex items-center gap-2'>
          ...
          <h2 className='text-lg font-semibold'>
            {t('settings:observability.title', 'Model Observability')}
          </h2>
        </div>
        <div className='flex items-center gap-2'>
          {/* Time Range Selector */}
      ...
      {showAlert && alertLevel && (
        ...
        >
          ...
          <div className='flex-1'>
            ...
            >
              {t(
                'settings:observability.alertMessage',
                'Your spending of {{amount}} exceeds the {{level}} threshold of {{threshold}}.',
                {
                  amount: formatCost(totalSpend.totalCostUsd),
                  level: alertLevel,
                  threshold: formatCost(
                    alertLevel === 'critical' ? COST_THRESHOLDS.critical : COST_THRESHOLDS.warning,
                  ),
                },
              )}
            </p>
      ...
      <div className='grid grid-cols-2 gap-4 sm:grid-cols-4'>
        <div className='rounded-lg border bg-card p-4'>
          <div className='flex items-center gap-2 text-muted-foreground'>
            ...
            <span className='text-xs font-medium'>
              {t('settings:observability.totalSpend', 'Total Spend')}
            </span>
          </div>
        ...
        <div className='rounded-lg border bg-card p-4'>
          <div className='flex items-center gap-2 text-muted-foreground'>
            ...
            <span className='text-xs font-medium'>
              {t('settings:observability.apiCalls', 'API Calls')}
            </span>
          </div>
        ...
        <div className='rounded-lg border bg-card p-4'>
          <div className='flex items-center gap-2 text-muted-foreground'>
            ...
            <span className='text-xs font-medium'>
              {t('settings:observability.totalTokens', 'Total Tokens')}
            </span>
          </div>
          <p className='mt-1 text-2xl font-bold'>
            {loading
              ? '...'
              : formatTokens(agentSummaries.reduce((sum, a) => sum + a.totalTokens, 0))}
          </p>
        </div>
        <div className='rounded-lg border bg-card p-4'>
          <div className='flex items-center gap-2 text-muted-foreground'>
            ...
            <span className='text-xs font-medium'>
              {t('settings:observability.avgLatency', 'Avg Latency')}
            </span>
          </div>
          <p className='mt-1 text-2xl font-bold'>
            {loading
              ? '...'
              : `${Math.round(agentSummaries.reduce((sum, a) => sum + a.avgLatencyMs, 0) / Math.max(agentSummaries.length, 1))}ms`}
          </p>
        </div>
      </div>
                ...
                <div key={agent.agent} className='space-y-1'>
                  <div className='flex items-center justify-between text-sm'>
                    ...
                    <span className='tabular-nums text-muted-foreground'>
                      {formatCost(agent.totalCostUsd)} · {formatTokens(agent.totalTokens)} tokens ·{' '}
              ...
              <thead>
                <tr className='border-b text-left text-muted-foreground'>
                  ...
                  <th className='pb-2 font-medium text-right'>
                    {t('settings:observability.calls', 'Calls')}
                  </th>
                  <th className='pb-2 font-medium text-right'>
                    {t('settings:observability.tokens', 'Tokens')}
                  </th>
                  <th className='pb-2 font-medium text-right'>
                    {t('settings:observability.cost', 'Cost')}
                  </th>
                </tr>
              </thead>
              <tbody>
