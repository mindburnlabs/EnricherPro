...
export const ConflictResolver: React.FC<ConflictResolverProps> = ({ fieldKey, evidence, onResolve }) => {
    const { t } = useTranslation(['detail', 'common']);

    // If no conflicts, return null
    if (!evidence.is_conflict || !evidence.conflicting_values || evidence.conflicting_values.length === 0) {
        return null;
    }
    ...
    return (
        <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-900/30 rounded-lg p-4 mb-4">
            ...
            <p className="text-xs text-amber-700 dark:text-amber-300 mb-4">
                {t('governance.conflict_desc', 'Multiple trusted sources reported different values for this field. Please select the correct value.')}
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="bg-white dark:bg-gray-800 p-3 rounded border border-gray-200 dark:border-gray-700 relative group hover:border-blue-400 transition-colors">
                    ...
                    <div className="flex items-center gap-2 text-xs text-gray-500 mb-3">
                        {currentValue.source ? (
                            <a href={currentValue.source} target="_blank" rel="noreferrer" className="flex items-center gap-1 hover:text-blue-500">
                ...
                {evidence.conflicting_values.map((conflict, idx) => (
                    <div key={idx} className="bg-white dark:bg-gray-800 p-3 rounded border border-amber-200 dark:border-amber-900/50 relative group hover:border-amber-400 transition-colors">
                        ...
                        <div className="flex items-center gap-2 text-xs text-gray-500 mb-3">
                            {conflict.source?.url ? (
                                <a href={conflict.source.url} target="_blank" rel="noreferrer" className="flex items-center gap-1 hover:text-blue-500">
                                    <ExternalLink className="w-3 h-3" />
