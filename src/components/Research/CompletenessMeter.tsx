...
export const CompletenessMeter: React.FC<CompletenessMeterProps> = ({ data, compact = false }) => {
  ...
  const analysis = useMemo(() => {
    const sections = {
      identity: {
        ...
        fields: [
          ...
          { key: 'mpn_identity.mpn', weight: 10, required: true },
          {
            ...
            weight: 5,
            required: true,
            check: (d: ConsumableData) => d.images && d.images.length > 0,
          },
      ...
      compatibility: {
        ...
        fields: [
          {
            key: 'compatible_printers_ru',
            weight: 20,
            required: true,
            check: (d: ConsumableData) =>
              d.compatible_printers_ru &&
              Array.isArray(d.compatible_printers_ru) &&
              d.compatible_printers_ru.length > 0,
          },
        ],
      },
      logistics: {
      ...
      compliance: {
        ...
        total: 0,
        missing: [] as string[],
        fields: [{ key: 'compliance_ru.tn_ved_code', weight: 5, required: true }],
      },
    };
  ...
  if (compact) {
    return (
      <div
        className={`flex items-center gap-1.5 px-2 py-1 rounded-md border ${percentage >= 80 ? 'border-emerald-200 dark:border-emerald-800' : 'border-amber-200 dark:border-amber-800'} ${bgClass}`}
      >
        ...
        <span
          className={`text-xs font-bold ${percentage >= 80 ? 'text-emerald-700 dark:text-emerald-300' : 'text-amber-700 dark:text-amber-300'}`}
        >
          {percentage}%
        </span>
      </div>
    ...
    <div className='mb-6 bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 border border-gray-100 dark:border-gray-800'>
      <div className='flex items-center justify-between mb-2'>
        <div className='flex items-center gap-2'>
          <h3 className='text-sm font-semibold text-gray-900 dark:text-gray-100'>
            {t('completeness.title', 'Data Completeness')}
          </h3>
          <span
            className={`text-xs font-bold px-2 py-0.5 rounded-full ${bgClass} ${colorClass.split(' ')[0].replace('text-', 'text-')}`}
          >
            {percentage}%
          </span>
        </div>
      ...
      {analysis.missing.length > 0 && (
        <div className='flex flex-wrap gap-1.5'>
          <span className='text-[10px] uppercase font-bold text-gray-400 self-center mr-1'>
            {t('completeness.missing', 'Missing')}:
          </span>
          {analysis.missing.slice(0, 4).map((key) => (
            <span
              key={key}
              className='px-1.5 py-0.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded text-[10px] text-gray-500'
            >
              {t(key)}
            </span>
          ))}
          {analysis.missing.length > 4 && (
            <span className='text-[10px] text-gray-400 self-center'>
              +{analysis.missing.length - 4} more
            </span>
          )}
        </div>
      )}
