...
export class OzonExporter implements IMarketplaceExporter {
  ...
  validate(item: EnrichedItem): string[] {
    ...
    if (!item.data.supplier_title_raw && !item.data.mpn_identity.canonical_model_name)
    ...
    const ozonPayload = {
      attributes: [
        ...
        },
        {
          ...
          id: 9048, // Name
          values: [
            { value: item.data.supplier_title_raw || item.data.mpn_identity.canonical_model_name },
          ],
        },
        {
          complex_id: 0,
