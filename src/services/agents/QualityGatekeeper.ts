...
export class QualityGatekeeper {
  static async validate(
    ...
    const MSG = {
      BRAND_UNKNOWN: isRu
        ? `Бренд '${data.brand}' не найден в списке известных`
        : `Brand '${data.brand}' not in common list`,
      MISSING_MPN: isRu
        ? 'ОТСУТСТВУЕТ_АРТИКУЛ: Продукт идентифицирован только по имени'
        : 'MISSING_MPN: Product identified by name only',
      INCONSISTENT_LOGISTICS: isRu
        ? 'НЕСООТВЕТСТВИЕ_ЛОГИСТИКИ: Есть габариты, но нет веса'
        : 'INCONSISTENT_LOGISTICS: Dimensions exist but weight is missing',
      MISSING_LOGISTICS: isRu
        ? 'НЕТ_ЛОГИСТИКИ: Вес/Габариты не найдены (NIX.ru)'
        : 'MISSING_LOGISTICS: Weight/Dimensions not found',
      MISSING_COMPATIBILITY: isRu
        ? 'НЕТ_СОВМЕСТИМОСТИ: Принтеры не найдены'
        : 'MISSING_COMPATIBILITY: No compatible printers found',
      ...
      SINGLE_SOURCE: isRu
        ? 'ОДИН_ИСТОЧНИК: Данные из одного магазина без подтверждения'
        : 'SINGLE_SOURCE: Data relies on a single retail source without official verification',
      NO_SOURCES: isRu
        ? 'НЕТ_ИСТОЧНИКОВ: Данные не подтверждены'
        : 'NO_SOURCES: Data has no verifiable sources',
      POSSIBLE_DUPLICATE: isRu
        ? 'ВОЗМОЖНЫЙ_ДУБЛИКАТ: Найден похожий товар'
        : 'POSSIBLE_DUPLICATE: Found similar item',
      STRICT_FAIL_LOGISTICS: isRu
        ? 'СТРОГИЙ_ОТКАЗ: Нет Веса/Габаритов'
        : 'STRICT_FAIL: Missing Weight/Dims',
      STRICT_FAIL_COMPAT: isRu
        ? 'СТРОГИЙ_ОТКАЗ: Нет Совместимости'
        : 'STRICT_FAIL: Missing Compatibility',
      STRICT_FAIL_SOURCES: isRu
        ? 'СТРОГИЙ_ОТКАЗ: Недостаточно источников'
        : 'STRICT_FAIL: Insufficient Sources',
    };
    ...
    if (
      data.brand &&
      knownBrands.some((b) => data.brand?.toUpperCase().includes(b.toUpperCase()))
    ) {
      stages.brand = true;
    } else {
    ...
    const consistencyPass = true;

    // 3a. Weight vs Dimensions
    const hasWeight = !!(
      data.packaging_from_nix?.weight_g ||
      (data as any).packaging?.package_weight_g ||
      (data as any).logistics?.weight
    );
    ...
    if (
      ...
      (data as any).compatibility_ru?.printers &&
      Array.isArray((data as any).compatibility_ru.printers) &&
      (data as any).compatibility_ru.printers.length > 0
    ) {
      stages.compatibility = true;
    } else {
    ...
    if (!hasIdentityEvidence) {
      ...
      warnings.push(MSG.UNATTRIBUTED_ID);
    }
    if (!hasCompatEvidence && stages.compatibility) {
      attributionPass = false;
      warnings.push(MSG.UNATTRIBUTED_COMPAT);
    }
    ...
    const isOfficialOrNix =
      uniqueDomains.size > 0 &&
      Array.from(uniqueDomains).some(
        (d) =>
          d.includes('hp.com') ||
          d.includes('canon') ||
          d.includes('kyocera') ||
    ...
    if (data.mpn_identity?.mpn) {
      try {
        ...
        const duplicate = await import('../backend/DeduplicationService.js')
          .then((m) => m.DeduplicationService.findPotentialDuplicate(data.mpn_identity!.mpn!))
          .catch(() => null);
        ...
        console.warn('Dedupe check failed silently', e);
      }
    }
    ...
    const isValid =
      stages.identity && stages.logistics && stages.compatibility && stages.completeness;
