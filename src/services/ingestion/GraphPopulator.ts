...
export class GraphPopulator {
     ...
     */
    static async populateFromResearch(itemId: string, data: Partial<ConsumableData>, jobId: string): Promise<{
        ...
        try {
            await db.transaction(async (tx) => {
                ...
                if (data.brand) {
                    ...
                    const edgeCreated = await this.upsertEdge(tx, consumableEntity.id, brandEntity.id, 'MANUFACTURED_BY');
                ...
                for (const printerName of compatiblePrinters) {
                    ...
                    const edgeCreated = await this.upsertEdge(tx, consumableEntity.id, printerEntity.id, 'COMPATIBLE_WITH');
                    if (edgeCreated) edgesCreated++;
                }
            ...
            console.log(`[GraphPopulator] Completed: ${entitiesCreated} entities, ${edgesCreated} edges created`);
        } catch (error) {
    ...
    private static async upsertConsumableEntity(
        ...
        const canonicalName = data.marketing?.seo_title || (data as any).title || `${data.brand || ''} ${mpn}`.trim();

        // Check if alias already exists
        const [existing] = await tx.select()
            .from(aliases)
            .where(and(
                eq(aliases.alias, normalizedMpn),
                eq(aliases.aliasType, 'exact')
            ))
            .limit(1);
    ...
    private static async upsertBrandEntity(
        ...
        const [existing] = await tx.select()
            .from(aliases)
            .where(and(
                eq(aliases.alias, normalizedBrand),
                eq(aliases.aliasType, 'exact')
            ))
            .limit(1);
    ...
    private static async upsertPrinterEntity(
        ...
        const [existing] = await tx.select()
            .from(aliases)
            .where(and(
                eq(aliases.alias, normalizedPrinter),
                eq(aliases.aliasType, 'exact')
            ))
            .limit(1);
    ...
    private static async upsertEdge(
        ...
        fromEntityId: string,
        toEntityId: string,
        type: 'COMPATIBLE_WITH' | 'MANUFACTURED_BY' | 'ALSO_KNOWN_AS' | 'REPLACED_BY' | 'PART_OF_SERIES'
    ): Promise<boolean> {
        const [existing] = await tx.select({ id: edges.id })
            .from(edges)
            .where(and(
    ...
    private static async linkEvidenceFromClaims(
        ...
        const itemClaims = await tx.select({
    ...
    private static extractAliases(data: Partial<ConsumableData>): { value: string; type: 'exact' | 'weak_signal' }[] {
