...
export class CompatibilityGraphBuilder {
     ...
     */
    static async build(skuId: string, printers: string[], sourceUrl: string, brand: string = 'Unknown') {
        if (!printers || printers.length === 0) return;

        console.log(`[CompatibilityGraphBuilder] Processing ${printers.length} printers for SKU ${skuId}`);
        ...
        for (const printerRaw of printers) {
            ...
            const existing = await db.query.printerModels.findFirst({
                where: and(
                    eq(printerModels.modelName, printerName),
                    eq(printerModels.brand, brand)
                )
            });
            ...
            if (existing) {
                ...
                try {
                    const [inserted] = await db.insert(printerModels).values({
                        brand: brand,
            ...
            if (printerId) {
                ...
                try {
                    await db.insert(skuPrinterCompatibility).values({
                        skuId: skuId,
                        printerId: printerId,
