  ...
  static async complete(config: CompletionConfig): Promise<string> {
    ...
    if (!SecurityUtils.validateApiKey(apiKey, 'OpenRouter')) {
      ...
      if (process.env.NODE_ENV === 'development' && process.env.OPENROUTER_API_KEY) {
        logger.warn(
          '⚠️ Using OPENROUTER_API_KEY from env (Dev Mode Fallback). UI Settings preferred for Production.',
        );
        // @ts-ignore
    ...
    if (targetModel === 'openai/gpt-5.2') {
      logger.warn(
        "LLM Service: Detected invalid model 'openai/gpt-5.2'. Auto-correcting to 'openrouter/auto'.",
      );
    ...
    if (config.models && config.models.length > 0) {
      ...
      fallbackModels = MODEL_CONFIGS[config.profile].candidates
          ...
          try {
            ...
            if (!response.ok) {
              ...
              if (response.status === 429 && targetModel.endsWith(':free')) {
                logger.warn(
                  `LLM Service: Rate Limit (429) on ${targetModel}. Error Body: ${errText}`,
                );
              } else {
        ...
        },
        {
          ...
          shouldRetry: (err) => {
            ...
            if (err.message.includes('Rate Limit') || err.message.includes('(429)')) {
              ...
              const isFreeOrAuto =
                targetModel.endsWith(':free') || targetModel === 'openrouter/auto';
              if (isFreeOrAuto) {
    ...
    try {
      ...
      if (
        e.message?.includes('Auth/Credit') ||
        ...
        if (config.onLog)
          config.onLog(`system`, `⚠️ LLM Service: ${limitMsg}. Initiating failover...`);
        ...
        if (nextModel && nextModel !== currentModelId) {
          logger.warn(`LLM Service: Switching attempt from ${currentModelId} to ${nextModel}`, {
      ...
      if (e.message?.includes('(400)') && body.response_format?.type === 'json_schema') {
        logger.warn(
          `LLM Service: 400 Error with json_schema. Retrying with RAW prompt fallback... ${e.message}`,
        );
