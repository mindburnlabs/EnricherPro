...
export class BackendFirecrawlService {
  ...
  private static getClient(apiKey?: string) {
    if (!apiKey)
      throw new Error('Missing Firecrawl API Key. Please configure it in the UI Settings.');
      ...
      async () => {
        ...
        try {
          ...
          if ((error as any).statusCode === 402 || (error as any).statusCode === 401) {
            console.warn(
              `Firecrawl Search disabled: ${(error as any).message || 'Insufficient Credits'}`,
            );
            return [];
          }
  ...
  static async scrape(
    ...
    try {
      ...
      console.warn(
        `[Firecrawl] Outputting Local Fallback for ${url} due to error: ${error.message}`,
      );
  ...
  static async agent(
    prompt: string,
    options: { apiKey?: string; timeout?: number; schema?: any } = {},
  ) {
    const { withRetry } = await import('../../lib/reliability.js');
    ...
    ); // Agent is expensive, retry carefully
  }
  /**
