...
import type {
  ...
  ModelUsageSummary,
  JobUsageSummary,
} from '../../types/observability_types.js';
...
export class ObservabilityService {
   ...
   */
  static async getAgentSummaries(
    ...
    const rows = await db
      ...
      .from(modelUsage)
      .where(and(eq(modelUsage.tenantId, tenantId), gte(modelUsage.timestamp, since)))
      .groupBy(modelUsage.agent);
   ...
   */
  static async getModelSummaries(
    tenantId: string = 'default',
    days: number = 7,
  ...
  static async getTotalSpend(
    tenantId: string = 'default',
    days: number = 30,
  ): Promise<{ totalCostUsd: number; totalCalls: number }> {
    const since = new Date(Date.now() - days * 24 * 60 * 60 * 1000);
