...
export const aliases = pgTable(
  'aliases',
  {
    ...
    aliasType: text('alias_type', {
      enum: ['exact', 'regex', 'weak_signal', 'machine_generated'],
    }).notNull(),
...
export const edges = pgTable(
  ...
  (table) => ({
    uniqueEdge: uniqueIndex('unique_graph_edge').on(
      table.fromEntityId,
      table.toEntityId,
      table.type,
    ),
...
export const compatibilityEdges = pgTable(
  'compatibility_edges',
  {
    id: uuid('id').defaultRandom().primaryKey(),
...
export const evidenceRecords = pgTable(
  'evidence',
  {
    ...
    sourceType: text('source_type', {
      enum: ['official_website', 'marketplace', 'datasheet_pdf', 'forum', 'manual', 'generic'],
    }).notNull(),
...
// 3. Claim
export const enrichedClaims = pgTable(
  'claim',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    skuId: uuid('sku_id')
  ...
  (table) => ({
    ...
    uniqueClaim: uniqueIndex('claim_field_unique').on(
      table.skuId,
      table.fieldName,
      table.fieldValue,
    ),
    verifiedCheck: check(
      ...
      sql`status != 'verified' OR evidence_id IS NOT NULL`,
    ),
  }),
);
...
// 5. PrinterModel
export const printerModels = pgTable(
  'printer_model',
  {
    id: uuid('id').defaultRandom().primaryKey(),
...
export const skuPrinterCompatibility = pgTable(
  'sku_printer_compatibility',
  {
    skuId: uuid('sku_id')
      .references(() => skus.id, { onDelete: 'cascade' })
      .notNull(),
    printerId: uuid('printer_id')
      .references(() => printerModels.id, { onDelete: 'cascade' })
      .notNull(),
    evidenceId: uuid('evidence_id').references(() => evidenceRecords.id),
...
// 7. Similar Products
export const similarProducts = pgTable(
  'similar_products',
  {
    skuId: uuid('sku_id')
      .references(() => skus.id, { onDelete: 'cascade' })
      .notNull(),
    relatedSkuId: uuid('related_sku_id')
      .references(() => skus.id, { onDelete: 'cascade' })
      .notNull(),
