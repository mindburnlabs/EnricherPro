...
export const skuEnrichment = inngest.createFunction(
    ...
    async ({ event, step, logger }) => {
        ...
        const initialExtraction = await step.run("extract-from-string", async () => {
            return await ExtractorAgent.extract(supplierString, { source: "supplier_string" }, (msg) => logger.info(msg));
        });
        ...
        const searchResults = await step.run("execute-search", async () => {
            ...
            const scraper = new (await import("../../services/backend/GenericScraper.js")).GenericScraper();
        ...
        const extractedClaims = await step.run("extract-claims", async () => {
            ...
            for (const result of searchResults) {
                // Save Evidence First
                const [evidence] = await db.insert(await import("../../db/schema.js").then(m => m.evidenceRecords)).values({
                ...
                if (data.tech_specs?.yield?.value) flatData.yield_amount = String(data.tech_specs.yield.value);
                if (data.logistics?.package_weight_g) flatData.weight_g = String(data.logistics.package_weight_g);

                for (const [key, value] of Object.entries(flatData)) {
                    if (value) {
                         const [claim] = await db.insert(await import("../../db/schema.js").then(m => m.enrichedClaims)).values({
                            skuId,
                            fieldName: key,
        ...
        await step.run("verify-resolve", async () => {
            ...
            for (const [field, verification] of Object.entries(report.fields)) {
                ...
                if (resolution.status === 'resolved' && resolution.resolvedValue) {
                     ...
                     if (field === 'weight_g') updatePayload.packageWeightG = parseInt(resolution.resolvedValue);
        ...
        await step.run("mine-compatibility", async () => {
            const { CompatibilityMinerAgent } = await import("../../services/agents/CompatibilityMinerAgent.js");
            const { CompatibilityGraphBuilder } = await import("../../services/logic/CompatibilityGraphBuilder.js");
            ...
            for (const res of results) {
                await CompatibilityGraphBuilder.build(skuId, res.printers, res.sourceUrl || "unknown", currentSku?.brand || 'Unknown');
            }
        });
