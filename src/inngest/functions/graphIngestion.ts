...
export const graphIngestionScheduled = inngest.createFunction(
    ...
    async ({ step }) => {
        ...
        const results = await step.run("process-batch", async () => {
            ...
            for (const item of candidateItems) {
                try {
                    ...
                    console.log(`[GraphIngestion] Item ${item.id}: ${result.entitiesCreated} entities, ${result.edgesCreated} edges`);
        ...
        console.log(`[GraphIngestion] Batch complete: ${results.processed} processed, ${results.errors} errors`);
