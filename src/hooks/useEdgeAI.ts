...
import { EdgeAIService, InferenceBackend, EmbeddingResult } from '../services/edge/EdgeAIService.js';
...
export interface UseEdgeAIResult {
    ...
    prefilter: (query: string, results: Array<{ id: string; title: string; snippet?: string }>) => Promise<string[]>;
...
export function useEdgeAI(autoInit = true): UseEdgeAIResult {
    ...
    const similarity = useCallback(async (text1: string, text2: string): Promise<number> => {
        if (!isReady) return 0;
        try {
    ...
    const prefilter = useCallback(async (
        query: string,
