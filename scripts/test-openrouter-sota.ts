...
async function runTest() {
  ...
  try {
    ...
    if (lastRequestBody) {
      ...
      if (lastRequestBody.model === 'openrouter/auto' || lastRequestBody.provider?.sort) {
        ...
        console.warn(
          '⚠️ Routing Strategy: Maybe default model used? Model: ' + lastRequestBody.model,
        );
      }
  ...
  try {
    ...
    if (lastRequestBody) {
      console.log(
        '🔍 Inspecting Request Body for Agent:',
      ...
      if (
        lastRequestBody.provider?.sort === 'price' ||
        lastRequestBody.provider?.sort === 'latency' ||
        lastRequestBody.provider?.order ||
        lastRequestBody.model.includes('auto') ||
        lastRequestBody.model.includes('nitro') ||
        lastRequestBody.provider === undefined
      ) {
