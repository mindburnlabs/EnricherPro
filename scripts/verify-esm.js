...
function checkFile(filePath) {
  ...
  lines.forEach((line, index) => {
    ...
    const checkPath = (importPath) => {
      ...
      if (!ext) {
        console.error(
          `❌ [Missing Extension] ${path.relative(ROOT_DIR, filePath)}:${index + 1} -> '${importPath}'`,
        );
        hasError = true;
      }
      ...
      try {
        ...
        if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
          console.error(
            `❌ [Directory Import] ${path.relative(ROOT_DIR, filePath)}:${index + 1} -> '${importPath}' (Must point to index.js)`,
          );
          hasError = true;
        }
      } catch (e) {
