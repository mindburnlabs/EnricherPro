...
function scoreModel(m: ORModel, strategy: string): number {
  ...
  const pIn =
    toNum((m.pricing as any)?.prompt ?? (m.pricing as any)?.input) ?? Number.POSITIVE_INFINITY;
  const pOut =
    toNum((m.pricing as any)?.completion ?? (m.pricing as any)?.output) ?? Number.POSITIVE_INFINITY;
...
function pickSmartFree(models: ORModel[], sort: 'context' | 'recency', limit: number): ORModel[] {
...
function pickMatchesFreeFirst(
  ...
  const paid = sortMatches(
    matches.filter((m) => !isFreeModel(m)),
    strategy,
  );
  ...
  for (const profile of Object.values(ModelProfile)) {
    ...
    for (const s of cfg.selectors) {
      ...
      if (s.kind === 'smart_free') {
        const picked = pickSmartFree(models, s.sort, s.limit ?? 1);
      ...
      if (s.kind === 'pattern') {
        const picked = pickMatchesFreeFirst(models, new RegExp(s.regex), s.strategy, s.limit ?? 1);
        selectedModels.push(...picked);
        continue;
      }
    }
    ...
    // Deduplicate by ID
    const uniqueModels = Array.from(new Set(selectedModels.map((m) => m.id))).map(
      (id) => selectedModels.find((m) => m.id === id)!,
    );
