...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  // Rate Limiting
  const ip =
    (request.headers['x-forwarded-for'] as string) || request.socket.remoteAddress || 'unknown';
  ...
  if (!process.env.INNGEST_EVENT_KEY) {
    ...
    return response
      .status(500)
      .json({ error: 'Configuration Error: Missing INNGEST_EVENT_KEY on server' });
  }
  ...
  try {
    ...
    try {
      ...
      return response
        .status(500)
        .json({ error: 'Failed to initialize research job', details: String(dbError) });
    }
    ...
    try {
      ...
      return response
        .status(500)
        .json({ error: 'Failed to trigger workflow', details: String(inngestError) });
    }
