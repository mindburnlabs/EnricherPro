...
async function verifySotaFeatures() {
    ...
    try {
        ...
        if (lastRequestBody.response_format?.type === 'json_schema') console.log("✅ JSON Schema Strict mode set");
        else console.error("❌ JSON Schema missing");

        if (lastRequestBody.plugins?.find((p: any) => p.id === 'response-healing')) console.log("✅ Response Healing plugin present");
